...
class Address(object):
    ...
    def nearby_stores(self, service='Delivery', closed=False):
        ...
        return [Store(x, self.country) for x in data['Stores']
                if x['IsOnlineNow'] and x['ServiceIsOpen'][service] or closed]

    def closest_store(self, service='Delivery', closed=False):
        stores = self.nearby_stores(service=service, closed=closed)
        if not stores:
